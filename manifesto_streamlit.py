...
years = st.sidebar.slider("Select years", min_value=2004, max_value=2024, value=(2004, 2009, 2014, 2019, 2024), step=5)
...
if compare_parties:
    ...
    bjp_topics = [x.strip() for topic in bjp_df[bjp_df['Year'].isin(years)]['Domains'].tolist() for x in topic.split(',')]
    ...
    inc_topics = [x.strip() for topic in inc_df[inc_df['Year'].isin(years)]['Domains'].tolist() for x in topic.split(',')]
    ...
    bjp_subcategories = [x.strip() for subcategory in bjp_df[bjp_df['Year'].isin(years)]['Topic_Subcategories'].tolist() for x in subcategory.split(',')]
    ...
    inc_subcategories = [x.strip() for subcategory in inc_df[inc_df['Year'].isin(years)]['Topic_Subcategories'].tolist() for x in subcategory.split(',')]
    ...
    bjp_summary_topics = [x.strip() for topic in bjp_df[bjp_df['Year'].isin(years)]['Summary_Topics'].tolist() for x in topic.split(',')]
    ...
    inc_summary_topics = [x.strip() for topic in inc_df[inc_df['Year'].isin(years)]['Summary_Topics'].tolist() for x in topic.split(',')]
    ...
    bjp_ner_list = [ner for ner_list in bjp_df[bjp_df['Year'].isin(years)]['NER'].tolist() for ner in eval(ner_list) if ner[0] not in stop_words + custom_stop_words]
    ...
    inc_ner_list = [ner for ner_list in inc_df[inc_df['Year'].isin(years)]['NER'].tolist() for ner in eval(ner_list) if ner[0] not in stop_words + custom_stop_words]
    ...
    # Display the most common topics as a radar chart
    st.subheader("Most Common Topics")
    topics = [x.strip() for topic in df[df['Year'].isin(years)]['Domains'].tolist() for x in topic.split(',')]
    ...
    # Display the most common subcategories
    st.subheader("Most Common Subcategories")
    subcategories = [x.strip() for subcategory in df[df['Year'].isin(years)]['Topic_Subcategories'].tolist() for x in subcategory.split(',')]
    ...
    # Display the most common summary topics
    st.subheader("Most Common Summary Topics")
    summary_topics = [x.strip() for topic in df[df['Year'].isin(years)]['Summary_Topics'].tolist() for x in topic.split(',')]
    ...
    # Display the most common named entities
    st.subheader("Most Common Named Entities")
    ner_list = [ner for ner_list in df[df['Year'].isin(years)]['NER'].tolist() for ner in eval(ner_list) if ner[0] not in stop_words + custom_stop_words]
